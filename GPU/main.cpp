...
void train_with_batches(Transformer& model, 
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (int batch = 0; batch < num_batches; batch++) {
            ...
            // Multiplicación matricial en GPU (ejemplo de integración)
            // Si tu modelo usa Matrix para forward/backward, modifica la multiplicación interna:
            // Ejemplo: Matrix output = input.cudaMultiply(weights);
            // Aquí solo mostramos cómo llamar, debes adaptar tu modelo para usar cudaMultiply en vez de *
            ...
            // ...output de batch desactivado para menos ruido...
        }
        ...
        std::cout << "Epoch " << (epoch + 1) << ": "
                  << epoch_duration.count() << "s | Loss: " << std::fixed << std::setprecision(5) << avg_loss
...
int main() {
    ...
    try {
        ...
        int epochs = 2;         // Menos épocas para pruebas rápidas
        int batch_size = 256;   // Batch más grande para acelerar
